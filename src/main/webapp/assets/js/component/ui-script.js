...
const krds_modal = {
	...
	init() {
		...
		const autoOpenModal = document.querySelector(".krds-modal.auto-open");

		if (autoOpenModal) {
			// 쿠키 확인 로직 추가
			if (!this.isModalHiddenToday(autoOpenModal.id)) {
				this.openModal(autoOpenModal.id);
			}
		}

		this.setupTriggers();
		this.setupTodayCloseHandlers();
	},

	// 오늘 모달이 숨겨졌는지 확인하는 메서드
	isModalHiddenToday(modalId) {
		const cookieName = `modal_${modalId}_hidden`;
		...
		for (let cookie of cookies) {
			...
			if (cookie.startsWith(`${cookieName}=`)) {
				...
				const hiddenDate = new Date(cookieValue);
				const today = new Date();

				// 쿠키의 날짜와 오늘 날짜 비교 (날짜만)
				return hiddenDate.toDateString() === today.toDateString();
			}
		}
	...
	// 오늘 하루 닫기 버튼 핸들러 설정
	setupTodayCloseHandlers() {
		const todayCloseButtons = document.querySelectorAll(".close-modal.today-close");
		...
		todayCloseButtons.forEach((button) => {
			button.addEventListener("click", (event) => {
				...
				if (modal) {
					...
					// 오늘 날짜로 쿠키 설정
					const today = new Date();
					document.cookie = `modal_${modalId}_hidden=${today.toDateString()}; path=/; expires=${new Date(
						today.getFullYear(),
						today.getMonth(),
						today.getDate() + 1
					).toUTCString()}`;
	...
	setupTriggers() {
		...
		this.modalCloseTriggers.forEach((trigger) => {
			trigger.addEventListener("click", (event) => {
				event.preventDefault();
				const modal = trigger.closest(".krds-modal"); // null 체크 추가
	...
	openModal(id) {
		...
		// inuix_scrollControl.lockScroll();
	...
	closeModal(id) {
		...
		// inuix_scrollControl.unlockScroll();
	...
	example() {
		this.datePickerArea.forEach((datePicker) => {
			...
			tblCells.forEach((cell) => {
				const day = cell.querySelector(".btn-set-date").innerText.trim().padStart(2, "0");
				...
				const formattedDate = `${String(numberYear).trim()}.${String(numberMonth)
					.padStart(2, "0")
					.trim()}.${day.trim()}`;
						...
						if (target === "확인") {
							if (type === "single") {
								...
								const value1 = action
									...
									?.getAttribute("data-date");
								const value2 =
									action.closest(".krds-calendar-area").querySelector("td.period.end")?.getAttribute("data-date") || "";
...
// 	init() {
// 		window.alert = this.customAlert.bind(this);
// 		window.confirm = this.customConfirm.bind(this);
// 	},

// 	generateModal(settings, type) {
// 		const uniqueId = `modal_alert_${Math.random().toString(36).substring(2, 9)}`;
// 		const template = document.createElement("section");
// 		template.className = "krds-modal modal-alert fade hide";
// 		template.setAttribute("role", "dialog");
// 		template.setAttribute("id", uniqueId);
// 		template.setAttribute("aria-labelledby", `tit_${uniqueId}`);
...
// 															<h2 id="tit_${uniqueId}" class="modal-title">
// 																	${settings.icon ? `<i class="ico ico-${settings.icon}-xl"></i>` : ""}
...
// 											<div class="conts-area text-center">
// 													${settings.message}
// 											</div>
// 									</div>
// 									<!-- //modal contents -->
...
// 											${
// 												type === "confirm"
// 													? `<button type="button" class="krds-btn medium tertiary action-cancel">
// 															${settings.cancelText || "취소"}
// 													</button>`
// 													: ""
// 											}
// 											<button type="button" class="krds-btn medium primary action-ok">
// 													${settings.okText || "확인"}
// 											</button>
// 									</div>
// 									<!-- //modal btn -->
...
// 	showModal(modal, focusedElement, callback, isAlert) {
...
// 			const focusables = modal.querySelectorAll('a, button, [tabindex="0"], input, textarea, select');
// 			setTimeout(() => {
// 				focusables[0].focus();
// 			}, 350);

// 			const modalContent = modal.querySelector(".modal-content");
// 			common.focusTrap(modalContent);
// 		}, 150);

// 		const handleOk = () => {
// 			this.closeModal(modal, focusedElement);
// 			if (isAlert && typeof callback === "function") {
// 				callback();
// 			} else if (!isAlert && typeof callback === "function") {
// 				callback(true);
// 			}
// 		};
...
// 		modal.querySelector(".action-ok").addEventListener("click", handleOk);
// 		modal.querySelector(".action-cancel")?.addEventListener("click", handleCancel);
...
// 	closeModal(modal, focusedElement) {
// 		const modalBack = modal.querySelector(".modal-back");
...
// 	customAlert(param, callback) {
// 		if (this.isAlertsOpen) return;
...
// 			okText: typeof param === "object" ? param.okText : "확인"
// 		};
...
// 		const modal = this.generateModal(settings, "alert");
// 		const focusedElement = document.activeElement;

// 		if (focusedElement?.tagName.toLowerCase() === "a") {
// 			focusedElement.addEventListener("click", (e) => e.preventDefault(), { once: true });
// 		}

// 		document.body.appendChild(modal);
// 		this.showModal(modal, focusedElement, callback, true);
// 	},

// 	customConfirm(param, callback) {
// 		if (this.isAlertsOpen) return;
// 		this.isAlertsOpen = true;

// 		if (typeof callback === "function") {
// 			this.asyncCustomConfirm(param)
// 				.then((result) => callback(result))
// 				.catch((error) => console.error("customConfirm Error:", error));
// 		} else {
// 			this.customAlert({
// 				icon: "error",
// 				title: '<span class="text-invalid">[I.N.UIX] Error</span>',
// 				message:
// 					"<p>confirm() 사용이 잘못되었습니다.<br> 콜백함수를 연결해 주세요.</p>" +
// 					'<p class="m-1"><a href="#" class="link">I.N.UIX Alerts 가이드 문서 참조하기</a></p>'
// 			});
// 		}
// 	},

// 	async asyncCustomConfirm(param) {
// 		const settings = {
// 			title: "",
// 			// icon: "success",
// 			message: typeof param === "string" ? param : param.message || "",
// 			okText: typeof param === "object" ? param.okText : "확인",
// 			cancelText: typeof param === "object" ? param.cancelText : "취소"
// 		};
...
// 		const modal = this.generateModal(settings, "confirm");
// 		const focusedElement = document.activeElement;
...
// 			this.showModal(modal, focusedElement, resolve);
// 		});
// 	},
...
/*** * inuix_scrollControl * ***/
const inuix_scrollControl = {
	originalBodyWidth: 0,
	scrollbarWidth: 0,

	init() {
		this.originalBodyWidth = document.body.clientWidth;
	...
	lockScroll() {
		const body = document.body;
		const header = document.querySelector(".fixed-header #krds-header");
		...
		if (this.scrollbarWidth > 0) {
			...
			if (header) {
				header.style.right = `${this.scrollbarWidth}px`;
			}
		}
	},
...
/*** * inuix_layerPopup * ***/
const inuix_layerPopup = {
	layerOpener: null,

	init() {
		// 초기에 show 클래스를 가진 레이어 찾아서 열기
		document.querySelectorAll(".layer-popup.show").forEach((popup) => {
			this.openLayer(popup, null);
		});

		// 닫기 버튼 이벤트 설정
		document.addEventListener("click", (event) => {
			const closeButton = event.target.closest(".btn-close-popup, .close-popup");
			if (closeButton && closeButton.closest(".layer-popup")) {
				event.preventDefault();
				const popup = closeButton.closest(".layer-popup");
				if (popup) {
					this.closeLayer("#" + popup.id, this.layerOpener);
				}
			}
		});

		// 전역 함수로 메서드 바인딩
		window.openLayer = this.openLayer.bind(this);
		window.closeLayer = this.closeLayer.bind(this);
	},

	getPosition(element) {
		// jQuery의 position()과 유사한 기능 구현
		const rect = element.getBoundingClientRect();
		const offsetParent = element.offsetParent || document.body;
		const parentRect = offsetParent.getBoundingClientRect();

		return {
			left: rect.left - parentRect.left - (offsetParent.scrollLeft || 0),
			top: rect.top - parentRect.top - (offsetParent.scrollTop || 0)
		};
	},

	openLayer(_target, _opener) {
		// _target이 문자열이 아닌 경우(예: DOM 엘리먼트)에도 처리
		const targetSelector = typeof _target === "string" ? _target : "#" + _target.id;
		const target = typeof _target === "string" ? document.querySelector(_target) : _target;

		if (!target) {
			alert("열 레이어를 올바로 지정해 주세요.");
			return;
		}

		// _opener가 문자열인 경우(ID 셀렉터)에도 처리
		const opener = typeof _opener === "string" ? document.querySelector(_opener) : _opener;
		this.layerOpener = opener;
		...
		setTimeout(() => {
			...
			if (opener) {
				const position = this.getPosition(opener);
				const outerHeight = opener.offsetHeight;

				target.style.left = position.left + "px";
				target.style.top = position.top + outerHeight + "px";
			}
		}, 100);

		setTimeout(() => {
			const popupInner = target.querySelector(".popup-inner");
			if (popupInner) {
				popupInner.setAttribute("tabindex", "0");
				popupInner.focus();
			}
			target.scrollTop = 0;
		}, 300);
	...
	closeLayer(_target, _opener) {
		...
		if (target.classList.contains("show")) {
			const openLayersCount = document.querySelectorAll(".layer-popup.show").length;
			if (openLayersCount === 1) {
				// 마지막 레이어가 닫힐 때 추가 작업이 필요하다면 여기에 작성
			}

			target.classList.add("hide");
			target.classList.remove("show");

			// opener가 있는 경우 포커스 처리
			if (opener) {
				opener.focus();
			} else if (this.layerOpener) {
				this.layerOpener.focus();
			}
		}
	}
};
...
/*** * inuix_datepicker * ***/
/*** * inuix_datepicker * ***/
const inuix_datepicker = {
	// 현재 열린 데이트피커의 대상 입력 필드 참조를 저장
	activeInputElement: null,
	// 화면 크기 기준점
	SCREEN_BREAKPOINT: 1024,
	// 현재 데이트피커가 모달 내부에 있는지 여부
	isInsideModal: false,

	init() {
		// jQuery를 사용하여 데이트피커 초기화
		$(document).ready(() => {
			setTimeout(() => {
				$(".form-datepicker")
					.each(function () {
						inuix_datepicker.setDatepicker($(this));
					})
					.on("click", function () {
						inuix_datepicker.removeDatepicker(); // 기존 데이트피커 제거 후 새로 열기
						inuix_datepicker.activeInputElement = $(this); // 현재 활성화된 입력 필드 저장

						// 모달 내부인지 확인
						inuix_datepicker.isInsideModal = $(this).closest(".modal, .popup, [role='dialog']").length > 0;

						inuix_datepicker.openCalendar($(this));
					});

				// 달력 버튼 클릭 이벤트 처리
				$(document).on("click", ".btn-datepicker", function (e) {
					e.preventDefault();
					inuix_datepicker.removeDatepicker(); // 기존 데이트피커 제거
					inuix_datepicker.activeInputElement = $(this).prev(); // 현재 활성화된 입력 필드 저장

					// 모달 내부인지 확인
					inuix_datepicker.isInsideModal = $(this).closest(".modal, .popup, [role='dialog']").length > 0;

					inuix_datepicker.openCalendar($(this).prev());
				});

				// 외부 클릭 시 데이트피커 닫기 기능 추가
				$(document).on("click", function (e) {
					if (inuix_datepicker.shouldCloseDatepicker(e.target)) {
						inuix_datepicker.removeDatepicker();
						inuix_datepicker.activeInputElement = null; // 참조 제거
						inuix_datepicker.isInsideModal = false;
					}
				});

				// 화면 크기 변경 시 위치 재조정
				$(window).on("resize", function () {
					if (inuix_datepicker.activeInputElement && $("#inseq-datepicker").length > 0) {
						inuix_datepicker.adjustDatepickerPosition();
					}
				});
			}, 200);
		});
	...
	setDatepicker(obj) {
		...
		$(obj).after(
			'<button type="button" class="btn btn-datepicker"><i class="svg-icon ico-datepicker"></i><span class="sr-only">달력</span></button>'
		);
	},

	// 외부 openCalendar 함수를 우회하여 안전하게 처리
	openCalendar(obj) {
		const $obj = $(obj);
		if (!$obj.length) return;

		const mode = $obj.data("mode") || "";
		const domElement = $obj[0];
		...
		try {
			...
			if (this.isInsideModal) {
				...
				div.style.zIndex = 9999; // 높은 z-index 설정
			} else {
			...
			// 원래 openCalendar 함수를 안전하게 호출
			// offsetLeft/offsetTop 오류를 방지하기 위해 필요한 값 미리 저장
			const originalGetObjectLeft = window.GetObjectLeft;
			const originalGetObjectTop = window.GetObjectTop;

			// 임시로 GetObjectLeft/GetObjectTop 함수 오버라이드
			window.GetObjectLeft = function () {
				return rect.left + (inuix_datepicker.isInsideModal ? 0 : scrollLeft);
			};

			window.GetObjectTop = function () {
				return rect.top + (inuix_datepicker.isInsideModal ? 0 : scrollTop);
			};

			// 기존 openCalendar 함수 호출
			window.openCalendar($obj, mode);

			// 원래 함수 복원
			window.GetObjectLeft = originalGetObjectLeft;
			window.GetObjectTop = originalGetObjectTop;

			// 모달 내부인 경우 z-index 및 위치 조정
			setTimeout(() => {
				this.handleModalDatepicker();
				if ($(window).width() > this.SCREEN_BREAKPOINT) {
					this.adjustDatepickerPosition();
				}
			}, 10);
		} catch (error) {
			console.error("데이트피커 표시 오류:", error);
			this.removeDatepicker();

			// 필요한 경우 fallback 처리
			this.createSafeDatepicker($obj, mode);
		}
	},

	// 안전한 fallback 데이트피커 생성
	createSafeDatepicker($obj, mode) {
		const rect = $obj[0].getBoundingClientRect();
		const $datepicker = $(
			'<div id="inseq-datepicker" class="inseq-datepicker">' +
				'<div style="padding: 15px; text-align: center;">달력을 불러오는 중 오류가 발생했습니다.</div>' +
				"</div>"
		);

		// 위치 및 스타일 설정
		$datepicker.css({
			position: this.isInsideModal ? "fixed" : "absolute",
			top: rect.bottom + "px",
			left: rect.left + "px",
			background: "#fff",
			...
			width: "300px",
			"z-index": this.isInsideModal ? 9999 : 1000
		});
	...
	// 모달 내 데이트피커 처리
	handleModalDatepicker() {
		const $datepicker = $("#inseq-datepicker");
		if (!$datepicker.length) return;

		if (this.isInsideModal) {
			// 모달 내부에 있는 경우
			const $modal = this.activeInputElement.closest(".modal, .popup, [role='dialog']");

			if ($modal.length) {
				// 모달의 z-index 가져오기
				let modalZIndex = parseInt($modal.css("z-index")) || 1000;

				// 데이트피커의 z-index를 모달보다 높게 설정
				$datepicker.css({
					"z-index": modalZIndex + 10,
					position: "fixed" // 모달 내부에서는 fixed 사용
				});
			}
		}
	},
	...
	adjustDatepickerPosition() {
		...
		const $inputField = this.activeInputElement;
		if (!$inputField || !$inputField.length) return;

		// 입력 필드의 위치와 크기 정보 가져오기
		const inputRect = $inputField[0].getBoundingClientRect();
		const datepickerHeight = $datepicker.outerHeight() || 300;
		const datepickerWidth = $datepicker.outerWidth() || 300;

		// 위치 기준 결정
		const isFixed = this.isInsideModal;
		...
		if (bottomSpace < datepickerHeight && inputRect.top > datepickerHeight) {
			// 위로 표시
			newTop = inputRect.top - datepickerHeight;
		} else {
			// 아래쪽에 표시
			newTop = inputRect.bottom;
		}

		// 오른쪽 공간 부족 여부 확인
		const rightSpace = windowWidth - inputRect.left;
		let newLeft;

		if (rightSpace < datepickerWidth) {
			// 오른쪽 공간이 부족하면 조정
			newLeft = Math.max(0, windowWidth - datepickerWidth - 10);
		} else {
			// 왼쪽에 표시
			newLeft = Math.max(0, inputRect.left);
		}
		...
		// 모달 내부일 경우 fixed 좌표, 그렇지 않으면 스크롤 고려
		if (isFixed) {
			css.top = newTop + "px";
			css.left = newLeft + "px";
		} else {
			css.top = newTop + scrollTop + "px";
			css.left = newLeft + scrollLeft + "px";
		}
	...
	// 데이트피커 외부 클릭 판단 함수
	shouldCloseDatepicker(target) {
		return !($(target).closest(".datepicker").length || $(target).closest("#inseq-datepicker").length);
	},

	// jQuery 플러그인 형태로도 사용할 수 있는 메서드
	asPlugin() {
		(function ($) {
			$.fn.inuixDatepicker = function (options) {
				// 옵션 설정 (필요한 경우)
...
window.addEventListener("DOMContentLoaded", () => {
	...
	// inuix 커스텀 컴포넌트
	// inuix_alerts.init();
	inuix_scrollControl.init();
	inuix_layerPopup.init();
	inuix_datepicker.init();
});
...
$(document).on("jqGridLoadComplete", function () {
	setTimeout(adjustJqGridHeaderPadding, 50);
});

//sweetalert2
$(document).on("click", "#alert-regist", function (e) {
	Swal.fire({
		title: "등록",
		html: "등록 되었습니다.",
		backdrop: false,
		...
		confirmButtonText: "<div style='font-size: 16px; color: #fff; padding: 2px 10px;'>" + "확인" + "</div>",

		//취소 버튼

		cancelButtonText: "<div style='font-size: 16px; color: #464C53; padding: 2px 10px;'>" + "취소" + "</div>"
	});
});

$(document).on("click", "#confirm-regist", function (e) {
	Swal.fire({
		title: "등록",
		html: "등록 하시겠습니까?",
		backdrop: false,
