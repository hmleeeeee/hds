...
function openCalendar(_obj, _mode) {
	...
	if ($(".datepicker-popup.show").length) {
		x = $(_obj).offsetParent().position().left;
		y = $(_obj).offsetParent().position().top + $(".datepicker-popup.show .popup-body").scrollTop();
	} else {
	...
	var arrValue = obj.value,
		_YY = parseInt(arrValue.substring(0, 4), 10),
		_MM = arrValue.substring(5, 7),
		_DD = arrValue.substring(8, 10);

	if (arrValue.length == 10) {
		if (checkNumber(_YY + _MM + _DD)) {
			day = _YY + "." + _MM + "." + _DD;

			showCalendar(_YY, _MM, _DD, " (현재선택)");
		}
	} else if (arrValue.length == 7) {
		if (checkNumber(_YY + _MM)) {
			day = _YY + "." + _MM;

			showCalendar(_YY, _MM, "01", " (현재선택)");
		}
	} else if (arrValue.length == 4) {
		if (checkNumber(_YY)) {
			day = _YY;

			showCalendar(_YY, "01", "01", " (현재선택)");
		}
	} else {
		var now = new Date();
		...
		day =
			now.getFullYear() +
			"." +
			(parseInt(now.getMonth() + 1, 10) < 10 ? "0" + (now.getMonth() + 1) : now.getMonth() + 1) +
			"." +
			(parseInt(now.getDate(), 10) < 10 ? "0" + now.getDate() : now.getDate());
...
window.showCalendar = function (_year, _month, _day, _stat) {
	if (_stat) {
		var _title = _stat;
	}
	if (document.getElementById("inseq-datepicker")) {
		...
		for (var i = document.getElementsByTagName("roundrect").length - 1; i > -1; i--) {
			document
				.getElementsByTagName("roundrect")
				[i].parentNode.removeChild(document.getElementsByTagName("roundrect")[i]);
		}
	...
	for (var i = parseInt(_year, 10) - 5; i <= parseInt(_year, 10) + 6; i++) {
		if (checkNum == 4 || checkNum == 7 || checkNum == 10) {
			tr = document.createElement("tr");
		}
			...
			if (firstWeek > 0) {
				...
				if (printDay > lastDay) {
					...
					a.title =
						_year +
						"." +
						(parseInt(_month, 10) < 10 ? "0" + parseInt(_month, 10) : _month) +
						"." +
						(parseInt(printDay + 1, 10) < 11 ? "0" + parseInt(printDay, 10) : printDay);
					a.onclick = new Function("clickCalendar(this);");
					a.innerHTML = printDay;

					if (
						_day == printDay &&
						_month == parseInt(day.split(".")[1], 10) &&
						_year == parseInt(day.split(".")[0], 10)
					) {
						a.setAttribute("class", "today");
						a.title += _title;
					}
	...
	if ($(".datepicker-popup.show").length) {
		var container = $(".datepicker-popup.show .popup-body");
...
window.clickCalendar = function (_obj, _mode) {
	if (_obj) {
		var inputVal;
		if (_mode == "month") {
		...
		// 값 변경 여부 감지를 위해 changed 데이터 속성을 true로 설정
...
window.showMonth = function (_year, _month, _day) {
...
window.showYear = function (_year, _month, _day) {
	showCalendar(_year, _month, _day);
	document.getElementById("tableCalendarTitle").style.display = "none";
	document.getElementById("tableCalendar").style.display = "none";
	document.getElementById("tableMonthTitle").style.display = "none";
...
// 			}
// 			obj.after(
// 				'<button type="button" class="btn btn-icon btn-datepicker"><i class="ico ico-calendar-lightgray-md"></i><span class="sr-only">달력</span></button>'
// 			);
// 		};
...
// 			openCalendar($(this));
// 			if ($(this).closest(".scroll-container").length) {
// 				var scrollLeft = $(this).closest(".scroll-container").scrollLeft();
// 				$("#inseq-datepicker").css("margin-left", -1 * scrollLeft);
// 			}
// 		});

// 		var shouldCloseDatepicker = function (target) {
// 			return !($(target).closest(".datepicker").length || $(target).closest("#inseq-datepicker").length);
// 		};
// 		$(document)
// 			.on("click", function (e) {
// 				if (shouldCloseDatepicker(e.target)) {
// 					removeDatepicker();
// 				}
// 			})
